...
# Count number of 'yes' and 'no' in the target variable
for i in range(1, 15):
    if data[i][5] == 'yes':
        yescount += 1
    else:
...
pyes = yescount / 14  # Probability of 'yes'
pno = nocount / 14    # Probability of 'no'
...
for i in range(4):
    ...
    # Calculate p(val) in the entire dataset
    for j in range(1, 15):
        if data[j][rnum] == val:
            pval += 1
    pval /= 14
    ...
    # Calculate p(val|yes) and p(val|no)
    for j in range(1, 15):
    ...
    pvalyes /= yescount  # Conditional probability p(val|yes)
    pvalno /= nocount    # Conditional probability p(val|no)

    # Store the conditional probabilities
    yesno[i][0] = pvalyes / pval if pval != 0 else 0  # p(val|yes) / p(val)
    yesno[i][1] = pvalno / pval if pval != 0 else 0    # p(val|no) / p(val)

# Calculate the final probabilities for 'yes' and 'no'
